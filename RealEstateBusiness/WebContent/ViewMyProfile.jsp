...
<%
	...
	System.out.println(u1.getImage());
	try {
		user.getName();
%>
  ...
  <form role="form" class="form-horizontal" action='UserController?page=Register' method='post' name="registrationForm"
    ...
    <div class="form-group">
     ...
     <!--<u1.getImage() %>  -->
       ...
       <img src="<%=u1.getImage() %>" class="img-circle" alt="problem uploading photo" width="304" height="236">
    </div>
    <div class="form-group">
      <label  for="reviewer">Role: </label>
      <div class="chechbox col-sm-offset-2">
      <label>Reviewer: </label><input type="text" value="<% if(("authenticated").equals(u1.getReviewer())){%><%="YES" %><%}else %><%="NO" %>" ><br>
      <label>Negotiator:  </label><input type="text" value="<% if(("authenticated").equals(u1.getNegotiator())){%><%="YES" %><%}else %><%="NO" %>" ><br>
      <label>Proprietor:  </label><input type="text" value="<% if(("authenticated").equals(u1.getProprietor())){%><%="YES" %><%}else %><%="NO" %>" ><br>
      </div>
    </div>
    <div class="form-group">
    ...
    <div class="col-sm-offset-3">
    	...
    	<button type="button" class="btn btn-success col-sm-offset-1" onclick='window.location.href="EditProfile.jsp"'>Edit Profile</button>
