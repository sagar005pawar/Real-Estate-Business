		...
		<div class="row">
			...
			<form action="BargainCntr?page=BidingPage&id=<%=asset.getId()%>" method="post">
				<div class="row col-sm-offset-1">
					...
					<%if(!user.getName().equals(asset.getSname())){ %>					
					<div class="row">
						<div class="col-sm-6" align="right"
							style="margin-top: 8px; font-size: 16">
							<b>Your Bid Amount:</b>
						...
						<div class="col-sm-6 well well-sm">
								<input type="number" min=<%=asset.getBprice() %> class="form-control" id="bidPrice" name="bidPrice" placeholder="Enter Bid" required="required" />
						</div>
					</div>
					<%} %>
					...
					<%} %>
						<button type="button" class="btn btn-success" onclick='window.location.href="BargainCntr?page=BuyerLink"'><b>Back</b></button>						
					</div>
				</div>
				</form>
			</div>
